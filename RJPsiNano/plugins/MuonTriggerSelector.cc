...
void MuonTriggerSelector::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
 ...
 //  if(debug) std::cout << "pass_dimuuon0 " << pass_dimuon0_path<<" pass_trk "<<pass_jpsiTrk_path<<std::endl;
  ...
  if(pass_dimuon01_path || pass_dimuon02_path || pass_jpsiTrk1_path || pass_jpsiTrk2_path || pass_jpsiTrk3_path || pass_jpsiTrk4_path || pass_jpsiTrk5_path || pass_jpsiTrk6_path || pass_dimuon0_jpsi_1_path || pass_dimuon0_jpsi_2_path || pass_dimuon43_jpsi_displaced_path ) {  
  ...
  for(const pat::Muon & muon : *muons)
  {
    ...
    {
      if(!dimuon0Flags[iTrg] && !jpsiTrkFlags[iTrg] && !jpsiTrk_PsiPrimeFlags[iTrg] && !jpsiTrk_NonResonantFlags[iTrg] && !dimuon0_jpsi_flags[iTrg] && !dimuon43_jpsi_displaced_flags[iTrg] ) continue;
    ...
    //save reco muon 
    if(recoMuonMatchingDimuon0_index != -1 || recoMuonMatchingJpsiTrk_index !=-1 || recoMuonMatchingJpsiTrk_PsiPrime_index !=-1 || recoMuonMatchingJpsiTrk_NonResonant_index !=-1 || recoMuonMatchingDimuon0_jpsi_index !=-1  || recoMuonMatchingDimuon43_jpsi_displaced_index !=-1 )
    {
